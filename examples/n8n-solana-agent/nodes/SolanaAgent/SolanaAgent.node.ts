...
class SolanaAgent implements INodeType {
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		const config = await getValidatedCredentials(this);
		...
		for (let i = 0; i < items.length; i++) {
			const operationResult = await processOperation(this, i, operation, agent);
...
async function getValidatedCredentials(executeFunctions: IExecuteFunctions): Promise<SolanaAgentConfig> {
